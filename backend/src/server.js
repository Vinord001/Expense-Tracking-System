...
import profileRoutes from "./src/routes/profile.js"; // ✅ Profile routes
...
app.get("/api/external/:name", async (req, res) => {
  const url = API_MAP[req.params.name];
  if (!url) return res.status(404).json({ message: "External API not found" });

  try {
    const response = await fetch(url);
    if (!response.ok)
      throw new Error(`Failed to fetch external API: ${req.params.name}`);
